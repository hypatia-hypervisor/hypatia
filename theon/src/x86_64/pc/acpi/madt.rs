...
pub(crate) fn parse(header: &Header, dp: *const u8) -> Result<CPUInventory> {
    ...
    // We overestimate the number of CPUs by assuming the
    // smaller LAPIC length.
    let mut cpus = Vec::with_capacity(datalen / ty::LAPIC_LEN);
    let mut ioapics = Vec::with_capacity(datalen / ty::IOAPIC_LEN);
...
fn parse_x2lapic(p: *const u8) -> Option<arch::ProcessorID> {
    ...
    ((flags.enabled() || flags.online_capable()) && id != 0xffffffff).then_some(arch::ProcessorID(id))
}
